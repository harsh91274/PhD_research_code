...
mastc(I2)=2;
% mastc(I2)=;        %overwriting tensile-shear to shear
...
sample_vol=newoutput(n1+1:n2+1,9);
pe=master2(:,8).*sample_vol;
pe2=sum(pe);
% sed=0.5.*master2(:,9).*master2(:,13);
% se=sed.*sample_vol;
% se2=sum(se);
sed=trapz(master2(:,9));
% master2(:,13));
...
disp('INPUT ENERGY');
disp(strcat('Input Strain Energy (from S1): ',num2str(se)));
disp(strcat('Input Potential Energy (from S3): ',num2str(pe2)));
disp(strcat('Total Input Energy: ',num2str(se+pe2)));
output=[output, se pe2 se+pe2]; 
%%
%master2
% 1 = cycle
% 2 = no of events
...
disp(strcat('Energy from Shear MF: ',num2str(e_shear),' Mega Joules'));
disp(strcat('Energy from Tensile-Shear MF: ',num2str(e_ts),' Mega Joules'));
disp(strcat('Energy from Tensile MF: ',num2str(e_tensile),' Mega Joules'));
e_sum=e_shear+e_ts+e_tensile; e_sf=(e_shear+e_ts)/e_sum;

output=[output, n_shear, n_ts, n_tensile, n_sum, n_sf, e_shear, e_ts, e_tensile, e_sum, e_sf];
%%
%Figure 1
f_color=linspecer(3);
f1_color=f_color;
...
%     ylim(af1(2),[0 1]);
    set(af1(2),'ytickmode','auto');
    hold on
    title(strcat('Total Energy Released: ',num2str(master2(end,6)),' Mega Joules'),'fontsize',12);
    set(gca,'yticklabel',num2str(get(gca,'ytick')','%.3f'));
%     set(gca,'xtick',[],'xcolor','b');
    xlabel('Strain','fontsize',12);
    ylabel('Energy(MJ)','fontsize',12,'color','k');
...
%     set(af1(2),'ycolor','b');
    ...
    title(strcat('Total Energy Released: ',num2str(master2(end,6)),' Mega Joules'),'fontsize',12);
    xlabel('Strain','fontsize',12);
    ylabel('Energy(Mega Joules)','fontsize',12,'color','k');
    set(gca,'yticklabel',num2str(get(gca,'ytick')','%.3f'));
...
while satisfied_ee==0
    ...
    ee_val=ee_fit(1);
    disp(strcat('Event Energy Rate: ',num2str(ee_val),' Mega Joules/ 1% strain'));
...
for i=n1:n2
    ...
    if pos_x~=0
        ...
        if pos_tpos~=0      %tensile
            plot(((pos_pm_t(:,3)+pos_pm_t(:,5))./2),((pos_pm_t(:,4)+pos_pm_t(:,6))./2),'x','color',cm(i-n1+1,:),'markersize',4,'linewidth',0.8); %this changed
            hold on
        end
        if pos_spos~=0  %shear
            plot(((pos_pm_s(:,3)+pos_pm_s(:,5))./2),((pos_pm_s(:,4)+pos_pm_s(:,6))./2),'o','color',cm(i-n1+1,:),'markersize',3,'linewidth',0.8);   %this chnaged
            hold on
        end
    end
...
for i=n1:n2
    ...
    if pos_x~=0
        ...
        if pos_tpos~=0      %tensile
            plot(((pos_pm_t(:,3)+pos_pm_t(:,5))./2),((pos_pm_t(:,4)+pos_pm_t(:,6))./2),'x','color',cm(i-n1+1,:),'markersize',4,'linewidth',0.8);
            hold on
        end
        if pos_spos~=0  %shear
            plot(((pos_pm_s(:,3)+pos_pm_s(:,5))./2),((pos_pm_s(:,4)+pos_pm_s(:,6))./2),'o','color',cm(i-n1+1,:),'markersize',3,'linewidth',0.8);
            hold on
...
title(strcat('Total Energy Released: ',num2str(sum(clump_events(:,8))),' Mega Joules'));
...
dlmwrite('bvalue.dat',bwrite,'delimiter','\t');
