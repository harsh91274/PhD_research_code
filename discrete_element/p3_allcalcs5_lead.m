...
for i=1:p1
    ...
    if e_option==1
        ...
        if master1(i,13)<=0 && master1(i,14)<=0
            master1(i,16)=abs(master1(i,14));
            master1(i,18)=0.1*0.5.*(1/cf).*((master1(i,16)./master1(i,17)).^2).*master1(i,17).*zave; %energy is in joules
            master1(i,20)=0.1*0.5.*(1/cf).*((master1(i,16)./master1(i,17)).^2).*zave; %energy is in joules/m2
        else
            master1(i,16)=master1(i,14);
            master1(i,18)=0.1*0.5.*(1/cf2).*((master1(i,16)./master1(i,17)).^2).*master1(i,17).*zave; %energy is in joules
            master1(i,20)=0.1*0.5.*(1/cf2).*((master1(i,16)./master1(i,17)).^2).*zave; %energy is in joules/m2
        end
    end
...
for i=n1:n2
    ...
    if pos_x~=0
        ...
        if pos_tpos~=0      %tensile
            plot(((pos_pm_t(:,3)+pos_pm_t(:,5))./2),((pos_pm_t(:,4)+pos_pm_t(:,6))./2),'x','color',cm(i-n1+1,:),'markersize',7,'linewidth',0.7); %this changed
            hold on
        end
        if pos_spos~=0  %shear
            plot(((pos_pm_s(:,3)+pos_pm_s(:,5))./2),((pos_pm_s(:,4)+pos_pm_s(:,6))./2),'o','color',cm(i-n1+1,:),'markersize',6,'linewidth',0.7);   %this chnaged
            hold on
        end
    end
...
for i=n1:n2
    ...
    if pos_x~=0
        ...
        if pos_tpos~=0      %tensile
            plot(((pos_pm_t(:,3)+pos_pm_t(:,5))./2),((pos_pm_t(:,4)+pos_pm_t(:,6))./2),'x','color',cm(i-n1+1,:),'markersize',7,'linewidth',0.7);
            hold on
        end
        if pos_spos~=0  %shear
            plot(((pos_pm_s(:,3)+pos_pm_s(:,5))./2),((pos_pm_s(:,4)+pos_pm_s(:,6))./2),'o','color',cm(i-n1+1,:),'markersize',6,'linewidth',0.7);
            hold on
...
export_fig fig29 f29_damage_parameter_all -q101 -painters -nocrop -pdf -jpeg -eps 
%%
% D-value
...
for i=n1:n2
    ...
    clump_master2(i-n1+1,5)=(0.67.*log(clump_master2(i-n1+1,4)))-2.9;
...
subplot(2,1,2)
f2=bar(clump_master2(:,2),clump_master2(:,5));
set(f1,'FaceColor','g','EdgeColor','k','linewidth',0.25);
hold on
xlabel('Axial Strain');ylabel('Moment');
