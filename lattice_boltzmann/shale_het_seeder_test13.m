...
layer_order0=[1 1 1 1 1];  %ALWAYS SMECTITE LAYER
layer_order00=[1 1 1 2 1];  %approximate to mineralogy fractions in core
layer_order000=[1 1 1 1 3];  %approximate to mineralogy fractions in core
...
%%
%make initial seed
[t, sh_init_seed, no_babies, ipore, smec_bed, no_babies1]=make_het_seed2(sigma,eps, seed_order);   %initial seed constructed
no_init_babies=no_babies1;
...
% frac_satisfied=0;
frac_satisfied=input('Add smectite layer (1=YES or 0=NO): ');
smec_add_count=0;
smec_bed_proxy=smec_bed;

while frac_satisfied==0
    bed2=circshift(smec_bed_proxy,[sigma sigma 0]);
    sh_init_seed=cat(3, sh_init_seed,ipore, bed2);
    smec_bed_proxy=bed2;
    smec_add_count=smec_add_count+1;
    
    no1=no1+(smec_add_count*no_babies1);
    vol_t1=no1*vol_smec;
    vol_grains=vol_t1+vol_t2+vol_t3+vol_t4;
    
    disp(strcat('Smectite beds added: ',num2str(smec_add_count)));
    disp(strcat('Smectite grains added: ',num2str(smec_add_count*no_babies1)));
    ...
    sh_init_phi=porosity_calc(sh_init_seed);    %porosity calculation
    disp(strcat('Porosity: ',num2str(sh_init_phi)));
    frac_satisfied=input('Satisfied (1=YES or 0=NO): ');
end
[m,~,~]=size(sh_init_seed);
...
while again==1
    ...
    eps1=eps+mag;
    sigma1=sigma+mag;
    [t, sh_pore_3d, no_babies3d, ipore3d, smec_bed3d, no_babies1_3d]=make_het_seed3(sigma1,eps1,seed_order, m, no_init_babies, eps);
    
    no1_3d=no1_3d+no_babies3d;
    vol_t1_3d=no1_3d*vol_smec;
    vol_t2_3d=no2*vol_ill;
    vol_t3_3d=no3*vol_chlo;
    vol_t4_3d=no4*vol_kao;
    vol_grains_3d=vol_t1_3d+vol_t2_3d+vol_t3_3d+vol_t4_3d;

    smec_bed3d_proxy=smec_bed3d;
    if smec_add_count~=0
        for i=1:smec_add_count
            bed3d=circshift(smec_bed3d_proxy,[sigma1 sigma1 0]);
            sh_pore_3d=cat(3, sh_pore_3d,ipore3d, bed3d);
            smec_bed3d_proxy=bed3d;
        end
        no1_3d=no1_3d+(smec_add_count*no_babies1_3d);
        vol_t1_3d=no1_3d*vol_smec;
        vol_grains_3d=vol_t1_3d+vol_t2_3d+vol_t3_3d+vol_t4_3d;
    end
    disp(strcat('Smectite beds added in Dilated seed: ',num2str(smec_add_count)));
    disp(strcat('Smectite grains added in Dilated seed: ',num2str(smec_add_count*no_babies1_3d)));
    
    volf1_3d=vol_t1_3d/vol_grains_3d;
    disp(strcat('Smectite Volume Fraction (Dilated Seed): ',num2str(volf1_3d)));
    volf2_3d=vol_t2_3d/vol_grains_3d;
    disp(strcat('Illite Volume Fraction (Dilated Seed): ',num2str(volf2_3d)));
    volf3_3d=vol_t3_3d/vol_grains_3d;
    disp(strcat('Chlorite Volume Fraction: ',num2str(volf3_3d)));
    volf4_3d=vol_t4_3d/vol_grains_3d;
    disp(strcat('Kaolinite Volume Fraction: ',num2str(volf4_3d)));
...
%%
function [t1, seed, no_babies, ipore1, smec_bed, no_babies1]= make_het_seed2(sigma,eps, seed_order)
smec_t=1; 
smec_l=5; 
...
thick_ind1=max(seed_order(1,:));
if thick_ind1==1
    seed_bed_thickness1=smec_t;
elseif thick_ind1==2
    seed_bed_thickness1=ill_t;
elseif thick_ind1==3
    seed_bed_thickness1=chlo_t;
else
    seed_bed_thickness1=kao_t;
end
...
for i=1:num_plates
    if seed_order1(i)==1
        seed_bed_length1=seed_bed_length1+smec_l;
    elseif seed_order1(i)==2
        seed_bed_length1=seed_bed_length1+ill_l;
    elseif seed_order1(i)==3
        seed_bed_length1=seed_bed_length1+chlo_l;
    else
        seed_bed_length1=seed_bed_length1+kao_l;
    end
end

seed_bed_length1=seed_bed_length1+((num_plates-1)*sigma);
t1=seed_bed_thickness1;
%%
%determine size of each bed
thick_ind2=max(seed_order(2,:));
if thick_ind2==1
    seed_bed_thickness2=smec_t;
elseif thick_ind2==2
    seed_bed_thickness2=ill_t;
elseif thick_ind2==3
    seed_bed_thickness2=chlo_t;
else
    seed_bed_thickness2=kao_t;
end
...
for i=1:num_plates
    if seed_order2(i)==1
        seed_bed_length2=seed_bed_length2+smec_l;
    elseif seed_order2(i)==2
        seed_bed_length2=seed_bed_length2+ill_l;
    elseif seed_order2(i)==3
        seed_bed_length2=seed_bed_length2+chlo_l;
    else
        seed_bed_length2=seed_bed_length2+kao_l;
    end
end

seed_bed_length2=seed_bed_length2+((num_plates-1)*sigma);
t2=seed_bed_thickness2;
%%
%determine size of each bed
thick_ind3=max(seed_order(3,:));
if thick_ind3==1
    seed_bed_thickness3=smec_t;
elseif thick_ind3==2
    seed_bed_thickness3=ill_t;
elseif thick_ind3==3
    seed_bed_thickness3=chlo_t;
else
    seed_bed_thickness3=kao_t;
end
...
for i=1:num_plates
    if seed_order3(i)==1
        seed_bed_length3=seed_bed_length3+smec_l;
    elseif seed_order3(i)==2
        seed_bed_length3=seed_bed_length3+ill_l;
    elseif seed_order3(i)==3
        seed_bed_length3=seed_bed_length3+chlo_l;
    else
        seed_bed_length3=seed_bed_length3+kao_l;
    end
end

seed_bed_length3=seed_bed_length3+((num_plates-1)*sigma);
t3=seed_bed_thickness3;

seed_bed_lengths=[seed_bed_length1 seed_bed_length2 seed_bed_length3];
[len_proxy,len_val]=max(seed_bed_lengths);
if len_val==1
    seed_bed_length=seed_bed_length1;
    bed1=zeros(seed_bed_length1,seed_bed_length1,seed_bed_thickness1);
    bed2=zeros(seed_bed_length1,seed_bed_length1,seed_bed_thickness2);
    bed3=zeros(seed_bed_length1,seed_bed_length1,seed_bed_thickness3);
    ipore1=zeros(seed_bed_length1,seed_bed_length1,eps, 1);
    ipore2=zeros(seed_bed_length1,seed_bed_length1,eps, 1);
elseif len_val==2
    seed_bed_length=seed_bed_length2;
    bed1=zeros(seed_bed_length2,seed_bed_length2,seed_bed_thickness1);
    bed2=zeros(seed_bed_length2,seed_bed_length2,seed_bed_thickness2);
    bed3=zeros(seed_bed_length2,seed_bed_length2,seed_bed_thickness3);
    ipore1=zeros(seed_bed_length2,seed_bed_length2,eps, 1);
    ipore2=zeros(seed_bed_length2,seed_bed_length2,eps, 1);
else
    seed_bed_length=seed_bed_length3;
    bed1=zeros(seed_bed_length3,seed_bed_length3,seed_bed_thickness1);
    bed2=zeros(seed_bed_length3,seed_bed_length3,seed_bed_thickness2);
    bed3=zeros(seed_bed_length3,seed_bed_length3,seed_bed_thickness3);
    ipore1=zeros(seed_bed_length3,seed_bed_length3,eps, 1);
    ipore2=zeros(seed_bed_length3,seed_bed_length3,eps, 1);
end
%%
%find proxy
...
% else
[babyseed1, baby_l, no_baby_beds1]=makefiller(smallest_grain, 1, seed_bed_thickness1,sigma, eps);
% no_gbabies1=1*no_baby_beds1;
[babx1, baby1, babyz1]=size(babyseed1);

repx=2*ceil(seed_bed_length/babx1);
repy=2*ceil(seed_bed_length/baby1);
bed1_proxy=repmat(babyseed1,[repx repy 1]);

smec_bed=bed1_proxy(1:seed_bed_length, 1:seed_bed_length, 1:seed_bed_thickness1);
...
% else
    [babyseed2, baby_l, no_baby_beds2]=makefiller(smallest_grain, 1, seed_bed_thickness2, sigma, eps);
%     no_gbabies2=1*no_baby_beds2;
% end
[babx2, baby2, babyz2]=size(babyseed2);

repx=2*ceil(seed_bed_length/babx2);
repy=2*ceil(seed_bed_length/baby2);

bed2_proxy=repmat(babyseed2,[repx repy 1]);
...
% else
    [babyseed3, baby_l, no_baby_beds3]=makefiller(smallest_grain, 1, seed_bed_thickness3, sigma, eps);
%     no_gbabies3=1*no_baby_beds3;
% end
[babx3, baby3, babyz3]=size(babyseed3);

repx=2*ceil(seed_bed_length/babx3);
repy=2*ceil(seed_bed_length/baby3);

bed3_proxy=repmat(babyseed3,[repx repy 1]);
...
function [t1, seed, no_babies, ipore1, smec_bed, no_babies1]= make_het_seed3(sigma,eps, seed_order,m, no_init_babies, eps_old)
smec_t=1; 
smec_l=5; 
...
%determine length of each bed
% seed_bed_length1=no_init_babies*smec_l+((no_init_babies-1)*sigma);
%%
%determine size of each bed
thick_ind2=max(seed_order(2,:));
if thick_ind2==1
...
% seed_bed_length3=no_init_babies*smec_l+((no_init_babies-1)*sigma);
% seed_bed_lengths=[seed_bed_length1 seed_bed_length2 seed_bed_length3];
% [~,len_val]=max(seed_bed_lengths);
seed_bed_length=m+(round(sqrt(no_init_babies))-1)*(eps-eps_old);

bed1=zeros(seed_bed_length,seed_bed_length,seed_bed_thickness1);
bed2=zeros(seed_bed_length,seed_bed_length,seed_bed_thickness2);
bed3=zeros(seed_bed_length,seed_bed_length,seed_bed_thickness3);
ipore1=zeros(seed_bed_length,seed_bed_length,eps, 1);
ipore2=zeros(seed_bed_length,seed_bed_length,eps, 1);
[~,num_plates]=size(seed_order);
t1=seed_bed_thickness2;
%%
%find proxy
smallest_grain=min(seed_order(1,:));
seq=findseq(seed_order1);
...
bed0=zeros(cubex, cubey, baby_t);
baby_ipore=zeros(cubex,cubey,eps);
...
if rem(t,(baby_t+eps))>=baby_t
    no_baby_beds=no_baby_beds+1;
end
...
if no_baby_beds>1
    ...
    for i=2:no_baby_beds;
        ...
        bed1=cat(3, bed1,baby_ipore, bed2);
        bed_proxy=bed2;
    end
else
     no_baby_beds=1;
