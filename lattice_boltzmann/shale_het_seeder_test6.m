...
layer_order0=[1 1 2 1 3 1 1];  %approximate to mineralogy fractions in core
layer_order00=[1 1 2 1 3 1 1];  %approximate to mineralogy fractions in core
layer_order000=[1 1 2 1 3 1 1];  %approximate to mineralogy fractions in core
...
[babx, baby, babyz]=size(babyseed);

repx=2*ceil(seed_bed_length/babx);
repy=2*ceil(seed_bed_length/baby);
bed1_proxy=repmat(babyseed,[repx repy 1]);

repx2=round(seed_bed_length/babx);
repy2=round(seed_bed_length/baby);

no_babies_proxy1=no_gbabies1*repx2*repy2;
...
bed1(:,:,1:babyz)=bed1_proxy(ind_adjust+1:seed_bed_length+ind_adjust, ind_adjust+1:seed_bed_length+ind_adjust, 1:babyz);
%%
fc1=0;  %fill counter
for i=1:num_plates
    if seed_order1(i)==1
...
no_babies_proxy2=no_gbabies2*repx2*repy2;
...
ind_adjust=rem(baby_ind,(baby_l+sigma));
bed2(:,:,1:babyz)=bed2_proxy(ind_adjust+1:seed_bed_length+ind_adjust, ind_adjust+1:seed_bed_length+ind_adjust, 1:babyz);
...
no_babies_proxy3=no_gbabies3*repx2*repy2;
...
ind_adjust=rem(baby_ind,(baby_l+sigma));
bed3(:,:,1:babyz)=bed3_proxy(ind_adjust+1:seed_bed_length+ind_adjust, ind_adjust+1:seed_bed_length+ind_adjust, 1:babyz);
% imaging(bed3);
%%
fc3=0;  %fill counter
for i=1:num_plates
...
cubex=len*baby_l + len*sigma;
cubey=len*baby_l + len*sigma;
cubez=t; 
...
fc0=0;
for i=1:len
    bed0(fc0+1: fc0+baby_l, fc0+1: fc0+baby_l, 1:baby_t)=1;
    fc0=fc0+baby_l;
    fc0=fc0+sigma;
end
...
baby_correction=round(no_ill*((ill_l^2)/((smec_l+sigma)^2)) + no_chlo*((chlo_l^2)/((smec_l+sigma)^2)) + no_kao*((kao_l^2)/((smec_l+sigma)^2)));  
